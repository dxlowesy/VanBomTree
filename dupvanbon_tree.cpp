...
void veb_tree_insert(Node* unode, int x, int xcnt)
{
	if (unode->minval == NIL)
	{
	...
	else
	{
		...
		if (unode->msize > 2)
		{
			...
			{
				veb_tree_insert(unode->summary, highx, 1);
				empty_insert(unode->cluster[highx], lowx);
			}
			else
			{
				veb_tree_insert(unode->cluster[highx], lowx, xcnt);
			}
		}
		if (x > unode->maxval)
		{
...
void veb_tree_all_delete(Node* unode, int x)
{
	...
	else
	{
		...
		if (x == unode->minval)
		{
			...
			unode->minval = x;
			unode->cntmin = unode->cluster[fir_cluster]->cntmin;
		}
		int highx = getHigh(x, divdx);
		int lowx = getLow(x, divdx);
		veb_tree_all_delete(unode->cluster[highx], lowx);
		if (getMin(unode->cluster[highx]) == NIL)
		{
			veb_tree_all_delete(unode->summary, highx);
			if (x == unode->maxval)
			{
				...
				if (maxcluster != NIL)
				{
					...
					unode->cntmax = unode->cluster[maxcluster]->cntmax;
				}
				else
				{
					unode->maxval = unode->minval;
					unode->cntmax = unode->cntmin;
				}
			}
		}
		else if (x == unode->maxval)
		{
			...
			unode->cntmax = unode->cluster[highx]->cntmax;
		}
	}
...
void veb_tree_delete(Node* unode, int x)
{
	...
	else
	{
		...
		{
			veb_tree_all_delete(unode->summary, highx);
			if (x == unode->maxval && unode->cntmax == 1)
			{
				int maxcluster = getMax(unode->summary);
				if (maxcluster != NIL)
				{
		...
		else if(x == unode->maxval && unode->cntmax == 1)
		{
			unode->maxval = getIndex(highx, unode->cluster[highx]->maxval, divdx);
			unode->cntmax = unode->cluster[highx]->cntmax;
		}
